...
public class UIAnchor : MonoBehaviour
{
	...
	public Vector3 GetPosition()
	{
		...
		if (DisableX)
		{
			vector3.x = mTrans.position.x;
		}
		if (DisableY)
		{
			vector3.y = mTrans.position.y;
		}
		return vector3;
	}
	...
	public void Update()
	{
		if (!(mAnim != null) || !mAnim.enabled || !mAnim.isPlaying)
		{
			...
			if (RoundToWhole)
			{
				localPosition.x = Mathf.Floor(mTrans.localPosition.x);
				localPosition.y = Mathf.Floor(mTrans.localPosition.y);
			}

...
public static class NGUIMath
{
	...
	public static Bounds CalculateRelativeWidgetBounds(Transform root, Transform child)
	{
		...
		if (componentsInChildren.Length != 0)
		{
			...
			{
				UIWidget uIWidget = componentsInChildren[i];
				if (!uIWidget.enabled || uIWidget.alpha == 0f)
				{
					continue;
				}
				if ((bool)uIWidget.containedBy)
				{
					if (hashSet.Contains(uIWidget.containedBy.transform))
					{
						continue;
					}
					Transform transform = uIWidget.containedBy.transform;
					bool flag2 = false;
					do
					{
						if (child == transform)
					...
					while ((bool)transform.parent && !flag2);
					if (flag2)
					{
						continue;
					}

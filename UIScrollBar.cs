...
public class UIScrollBar : MonoBehaviour
{
	...
	private void CenterOnPos(Vector2 localPos)
	{
		if (!(mBG == null) && !(mFG == null))
		{
			...
			if (mDir == Direction.Horizontal)
			{
				...
				float num4 = ((num > 0f) ? ((localPos.x - num3) / num) : 0f);
				scrollValue = (mInverted ? (1f - num4) : num4);
			}
			else
			{
				...
				float num8 = ((num5 > 0f) ? (1f - (localPos.y - num7) / num5) : 0f);
				scrollValue = (mInverted ? (1f - num8) : num8);
			}
		}
	}
	...
	public void ForceUpdate()
	{
		...
		float num = (mInverted ? (1f - mScroll) : mScroll);
		if (mDir == Direction.Horizontal)
		{

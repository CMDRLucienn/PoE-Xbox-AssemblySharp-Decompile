...
public abstract class UITweener : IgnoreTimeScale
{
	...
	private void Update()
	{
		float num = (ignoreTimeScale ? UpdateRealTimeDelta() : Time.deltaTime);
		float num2 = (ignoreTimeScale ? base.realTime : Time.time);
		if (!mStarted)
		{
			mStarted = true;
		...
		if (style == Style.Once && (mFactor > 1f || mFactor < 0f) && !AlwaysRunUpdate)
		{

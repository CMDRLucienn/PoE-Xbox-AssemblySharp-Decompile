...
public class UISprite : UIWidget
{
	...
	protected void SimpleFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		Color32 item3 = (atlas.premultipliedAlpha ? NGUITools.ApplyPMA(color) : color);
		cols.Add(item3);
		cols.Add(item3);
		cols.Add(item3);
		cols.Add(item3);
	...
	protected void SlicedFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		Color color = base.color;
		color.a *= mPanel.alpha;
		Color32 item = (atlas.premultipliedAlpha ? NGUITools.ApplyPMA(color) : color);
		for (int j = 0; j < 3; j++)
		{
			int num5 = j + 1;
			for (int k = 0; k < 3; k++)

...
public class UITable : MonoBehaviour
{
	...
	private float m_calcHeight;

	public float Height => m_calcHeight;
		...
		get
		{
			if (mChildren.Count == 0)
			{
				...
				if (sorted)
				{
					if (reverseSort)
					{
						mChildren.Sort(SortByNameReversed);
					}
					else
					{
	...
	public static int SortByNameReversed(Transform a, Transform b)
	{
	...
	private void RepositionVariableSize(List<Transform> children)
	{
		...
		int num4 = ((columns > 0) ? Mathf.Min(children.Count, columns) : children.Count);
		...
		for (int count = children.Count; i < count; i++)
		{
			...
			UITable component = obj.GetComponent<UITable>();
			if ((bool)component)
			{
		...
		for (int count2 = children.Count; j < count2; j++)
		{
			...
			if (++num5 >= columns && columns > 0)
			{
				...
				m_calcHeight = Mathf.Max(m_calcHeight, num2);
			}
		}
	}

...
public class UICamera : MonoBehaviour
{
	...
	public const float doubleClickMotionCap = 10f;
	...
	public static Vector2 lastLastTouchPosition = Vector2.zero;
	...
	public static bool DisableSelectionInput = false;
	...
	private bool currentTouchLeft => currentTouch == mMouse[0];

	private bool currentTouchRight => currentTouch == mMouse[1];

	private string currentTouchDesc
	{
		get
		{
			if (currentTouchLeft)
	...
	public event EventHandler OnPreRenderCallbacks;
	...
	public static bool Raycast(Vector3 inPos, out RaycastHit hit)
	{
		for (int i = 0; i < mList.Count; i++)
		{
			...
			float maxDistance = ((uICamera.rangeDistance > 0f) ? uICamera.rangeDistance : (currentCamera.farClipPlane - currentCamera.nearClipPlane));
	...
	public static UICamera FindCameraForLayer(int layer)
	{
		...
		if ((bool)nGUICamera)
		{
			return nGUICamera.GetComponent<UICamera>();
		}
		int num = 1 << layer;
		for (int i = 0; i < mList.Count; i++)
		{
	...
	private static bool IsSelectionInput(string funcName)
	{
		string text = funcName.ToLower();
		if (text.Contains("click") || text.Contains("press") || text.Contains("key") || text.Contains("select"))
		{
			return true;
		}
		return false;
	...
	{
		if ((!DisableSelectionInput || !IsSelectionInput(funcName)) && go != null)
		{
	...
	public void ProcessMouse()
	{
		...
		if (useMouse)
		{
			for (int l = 0; l < 3; l++)
			{
				bool flag4 = Input.GetMouseButtonDown(l) && !GameCursor.Instance.DisableCursor;
				...
				ProcessTouch(flag4, mouseButtonUp);
			}
			currentTouch = null;
		}
	...
	public void ReallowTooltip(GameObject hover)
	{
		if (mTooltipTime == 0f && mHover == hover)
		{
			mTooltipTime = Time.realtimeSinceStartup + tooltipDelay;
		}
	}
	...
	public void ProcessOthers()
	{
		...
		if (num != 0)
		{
			Notify(mSel, "OnKey", (num > 0) ? KeyCode.UpArrow : KeyCode.DownArrow);
		}
		if (num2 != 0)
		{
			Notify(mSel, "OnKey", (num2 > 0) ? KeyCode.RightArrow : KeyCode.LeftArrow);
		}
	...
	public void ProcessTouch(bool pressed, bool unpressed)
	{
		...
		for (int i = 0; i < mMouse.Length; i++)
		{
			if (mMouse[i] != currentTouch && mMouse[i].pressStarted)
			{
				return;
			}
		}
			...
			if (currentTouch.pressed != null)
			{
				...
				if (magnitude != 0f)
				{
					...
					if (currentTouch.dragStarted)
					{
						...
						Notify(currentTouch.dragged, "On" + currentTouchDesc + "Drag", currentTouch.delta);
						isDragging = false;
						if (num3)
						{
		...
		if (currentTouch.pressed != null)
		{
			...
			if (currentTouch.dragged == currentTouch.current || (currentTouch.clickNotification != 0 && currentTouch.totalDelta.magnitude < num))
			{
				if (currentTouch.pressed != mSel)
				{
					...
					Notify(currentTouch.pressed, "On" + currentTouchDesc + "Select", true);
				}
				else
				{
					mSel = currentTouch.pressed;
				...
				{
					float realtimeSinceStartup = Time.realtimeSinceStartup;
					Notify(currentTouch.pressed, "On" + currentTouchDesc + "Click", null);
					if (currentTouch.clickTime + 0.5f > realtimeSinceStartup && (currentTouch.pos - lastLastTouchPosition).magnitude <= 10f)
					{
						Notify(currentTouch.pressed, "On" + currentTouchDesc + "DoubleClick", null);
					}
					lastLastTouchPosition = lastTouchPosition;
					currentTouch.clickTime = realtimeSinceStartup;
				}
			}
			else
			{
				Notify(currentTouch.current, "On" + currentTouchDesc + "Drop", currentTouch.dragged);
	...
	{
		if (this.OnPreRenderCallbacks != null)
		{
			this.OnPreRenderCallbacks(this, null);
		}
	}
}
